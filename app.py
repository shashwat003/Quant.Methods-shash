...
# Same UI/code as before. Changes are LIMITED to chatbot logic so that:
# - Verification prompts appear ONLY for sensitive intents (lost/stolen/fraud/balance/etc).
# - General banking questions are answered directly without asking for identity.
...
# ────────────────────────────────────────────────────────────────────────────────
# PARSING & VERIFICATION HELPERS (unchanged from last fix)
# ────────────────────────────────────────────────────────────────────────────────
MONTHS = {
...
def clean_dob_text(s: str) -> str:
    x = human_dob(s)
    x = re.sub(r"\b(\d{1,2})(?:st|nd|rd|th)\b", r"\1", x)
...
def parse_identity(text:str, expect_acct:bool=False, expect_last4:bool=False):
    ...
    if expect_last4 and not last4:
        last4 = re.match(r"^\s*(\d{4})\s*$", t)
    acct = re.search(ACCT_P, tl)
    if expect_acct and not acct:
        acct = re.match(r"^\s*(\d{5,})\s*$", t)
...
# ────────────────────────────────────────────────────────────────────────────────
# STATE (added 'mode' to gate verification)
# ────────────────────────────────────────────────────────────────────────────────
def init_state():
    ...
    if "verif" not in st.session_state:
        st.session_state.verif={
            "mode":"idle",                 # <─ NEW: 'idle' or 'verify'
            "step":"await_name",
            "name":None,"acct":None,"last4":None,"dob":None,
            "tries":0,"ok":False,"record":None
        }
init_state()

# ────────────────────────────────────────────────────────────────────────────────
# HEADER + TICKER (unchanged)
# ────────────────────────────────────────────────────────────────────────────────
...
with right:
    ...
    if user_text:
        ...
        # Sensitive intents (gate verification)
        sensitive = any(k in low for k in [
            "lost", "stolen", "fraud", "dispute", "unauthorised", "unauthorized",
            "freeze", "block", "replace card", "new card", "card replacement",
            "balance", "statement", "account details", "card", "pin"
        ])

        # Start verification only if needed, or continue if already in verification
        if (not v["ok"]) and (v["mode"] == "verify" or sensitive):
            v["mode"] = "verify"  # enter/continue verification mode

            # Parse with step-awareness
            ...
            if step == "await_name":
                ...
                if v["dob"] and v["last4"] and v["name"]:
                    ...
                    if ok:
                        v["ok"]=True; v["record"]=rec; v["step"]="verified"; v["mode"]="idle"
        ...
        # Not in verification (general Q&A or already verified)
        reply = None
        if v["ok"]:
            # still do our local intent handling for verified users
            if "balance" in low:
                reply = f"Your balance is €{CUSTOMERS[v['record']['name'].lower()]['balance']:.0f}."
            elif any(k in low for k in ["lost","stolen","freeze","block","replace card","new card"]) and "card" in low:
                rec = v["record"]
        ...
        # If no local reply, answer general banking/finance via model (NO verification)
        if reply is None:
